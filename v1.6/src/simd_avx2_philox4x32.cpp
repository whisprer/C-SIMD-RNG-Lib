...
// Emulate high 32 bits of 32x32->64 for all 8 lanes
static inline __m256i ua_mulhi32_all(__m256i a, __m256i b) {
  __m256i prod02 = _mm256_mul_epu32(a, b);          // even 32-bit lanes as 64-bit
  __m256i a13 = _mm256_srli_epi64(a, 32);           // odd parts
  __m256i b13 = _mm256_srli_epi64(b, 32);
  __m256i prod13 = _mm256_mul_epu32(a13, b13);      // odd lanes as 64-bit

  __m256i hi02 = _mm256_srli_epi64(prod02, 32);
  __m256i hi13 = _mm256_srli_epi64(prod13, 32);

  // put odd highs into odd 32-bit lanes, blend with even highs
  __m256i hi13_shifted = _mm256_slli_epi64(hi13, 32);
  return _mm256_blend_epi32(hi02, hi13_shifted, 0xAA);
}

struct Philox4x32_10_AVX2 {
  // per-backend constants (scoped)
  ...
  static inline void round_once(__m256i& x0, __m256i& x1, __m256i& x2, __m256i& x3,
    ...
    const __m256i m0 = _mm256_set1_epi32(static_cast<int>(M0));
    const __m256i m1 = _mm256_set1_epi32(static_cast<int>(M1));

    __m256i lo0 = ua_mullo32(m0, x0);
    __m256i hi0 = ua_mulhi32_all(m0, x0);
    __m256i lo1 = ua_mullo32(m1, x2);
    __m256i hi1 = ua_mulhi32_all(m1, x2);
  ...
  inline void block8_store(std::uint64_t* dst) noexcept {
    ...
    for (int i=0; i<9; ++i) {
      ...
      k0v = _mm256_add_epi32(k0v, w0v);
      k1v = _mm256_add_epi32(k1v, w1v);
    }
    round_once(x0,x1,x2,x3, k0v, k1v);
